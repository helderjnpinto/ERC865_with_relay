...
contract('AlkiToken', accounts => {
  ...
  it('Bob sign transaction to relayer sending 10 tokens for alice giving 1 of fee', async () => {
    ...
    const encodedDataToSign = metaTransfer.encodeDataToSign(
      metaTx,
      alki.address
    )
    console.log('encodedDataToSign', encodedDataToSign)

    const encodedDomainData = metaTransfer.encodeDomainData(
      encodedDataToSign.domain,
      encodedDataToSign.types
    )
    console.log('encodedDomainData', encodedDomainData)

    const encodedMetaTransferTypeData = metaTransfer.encodeMetaTransferType(
        encodedDataToSign.types
    )
    // messageHash: 0x2b2a11e7771ec501c4d617e6e7ee9815934cb7013013fb237c87a8ca0a0154e1
    console.log("encodedMetaTransferTypeData", encodedMetaTransferTypeData)
      
    const encodedMetaTransferData = metaTransfer.encodeMetaTransferStruct(
        encodedDataToSign.message,
        encodedDataToSign.types
    )
    console.log('encodedMetaTransferData', encodedMetaTransferData)

    const encodePrimaryType = metaTransfer.encodePrimaryType(
      encodedDataToSign.primaryType,
      encodedDataToSign.message,
      encodedDataToSign.types
    )
    console.log('encodePrimaryType', encodePrimaryType)

    const messageSigned = metaTransfer.signTypedData_v4(
      PK_8_BOB,
      encodedDataToSign
    )
    console.log('messageSign', messageSigned)

    const recoverSigner = metaTransfer.recoverTypedSignature_v4(
      encodedDataToSign,
      messageSigned
    )
    console.log('recoverSigner', recoverSigner)

    const getSignatureParams = metaTransfer.getSignatureParameters(
      messageSigned
    )
    console.log('getSignatureParams', getSignatureParams)

    const receiptMeta = await alki.executeMetaTransfer(
      bob,
      alice,
      10,
      1,
    ...
    console.log('relayerBalance', relayerBalance.toString())
